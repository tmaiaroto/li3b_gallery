...
class ItemsController extends \lithium\action\Controller {
			...
			foreach($this->request->data['Filedata'] as $file) {
				...
				if ($gridFileId) {
					...
					if($document->save($data)) {
						FlashMessage::write('Successfully added item(s) to the gallery.', 'default');
					}
				}
			}
			return;
		}
		FlashMessage::write('Sorry, there was seemingly nothing to upload, please add some files and try again.', 'default');
	...
	public function admin_manage($id=null) {
		if(empty($id)) {
			FlashMessage::write('You must provide a gallery id to manage its items.', 'default');
	...
	*/
	...
	public function admin_clear_cache() {
		if(Thumbnail::clearCache()) {
			FlashMessage::write('Successfully cleared the thumbnail image cache.', 'default');
		} else {
			FlashMessage::write('Could not clear the thumbnail image cache, please try again.', 'default');
		}
